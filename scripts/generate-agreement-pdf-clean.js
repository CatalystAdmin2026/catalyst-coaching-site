// @ts-check
/**
 * Generates "Catalyst Coaching Master Client Agreement - Clean.pdf"
 * Identical layout to the branded template but with NO visible {{...}} placeholders.
 * Visual labels, underlines, and blank space are preserved for DocuSign field placement.
 * Run: node scripts/generate-agreement-pdf-clean.js
 */
...
const OUT_FILE = path.join(OUT_DIR, "Catalyst Coaching Master Client Agreement - Clean.pdf");
...
// All field values are blank — no {{...}} placeholders visible in the PDF.
// DocuSign fields will be placed manually over the blank space and underlines.
const FIELDS = {
  ClientName:        "",
  ClientEmail:       "",
  PackageName:       "",
...
// Agreement sections — {{...}} inline in body text replaced with blank underscores
const SECTIONS = [
  {
  ...
  },
  {
    ...
    body: 'This Agreement begins on _______________ ("Start Date") and continues on a month-to-month basis. Either party may cancel with fourteen (14) days written notice as outlined in Section 8.',
  },
  {
...
function generate() {
  ...
  doc.rect(MARGIN, BOX_Y, 2.5, BOX_H).fill(GOLD);
  ...
  clientRows.forEach(([label, val], i) => {
    ...
    // Draw a subtle underline for the blank field area
    doc.rect(MARGIN + 8 + 90, ry + 7, BOX_W - 106, 0.4).fill("#333333");
    if (val) {
      doc.fillColor(WHITE).fontSize(6).font("Helvetica").text(val, MARGIN + 8 + 92, ry, { lineBreak: false });
    }
  });
  ...
  adminRows.forEach(([label, val], i) => {
    ...
    doc.rect(IBX + 8 + 90, ry + 7, BOX_W - 106, 0.4).fill("#333333");
    if (val) {
      doc.fillColor(WHITE).fontSize(6).font("Helvetica").text(val, IBX + 8 + 92, ry, { lineBreak: false });
    }
  });
  ...
  // Client sig fields — underlines only, no placeholder text
  const sigFields = [
  ...
  // Coach sig fields — underlines only, static text for known values, blank for sig/date
  const coachFields = [
