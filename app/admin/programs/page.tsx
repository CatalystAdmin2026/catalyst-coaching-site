  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.ok && data.template) {
        window.location.href = `/admin/programs/${data.template.id}`;
      } else {
      ...
      <div className="min-h-screen bg-[#080909] text-white">
          ...
          <div className="max-w-screen-xl mx-auto px-4 md:px-8">
            <div className="flex items-center justify-between h-14">
              <div className="flex items-center gap-4">
                <Link href="/admin" className="text-gray-600 hover:text-gray-400 text-xs tracking-widest uppercase font-semibold transition-colors">
                  ← Dashboard
                </Link>
                <div className="w-px h-4 bg-white/10" />
                <div className="flex items-center gap-2">
                  <div className="w-1.5 h-5 bg-[#C9A24D] rounded-sm" />
                  <p className="text-white font-semibold text-sm tracking-wide">Program Builder</p>
                </div>
              </div>
              <button
        ...
        <div className="max-w-screen-xl mx-auto px-4 md:px-8 py-8">
          ...
          <div className="flex items-center gap-3 mb-6">
            <h2 className="text-[10px] tracking-[0.5em] text-gray-600 uppercase font-semibold">Training Programs</h2>
          ...
          {!loading && !error && programs.length === 0 && (
            <div className="border border-white/[0.06] border-dashed px-8 py-12 text-center">
              ...
              <p className="text-gray-700 text-xs">Click <button onClick={() => setShowForm(true)} className="text-[#C9A24D] hover:text-[#D4B56A]">New Program</button> to build your first multi-week program.</p>
            </div>
          )}
          {!loading && programs.length > 0 && (
            <div className="space-y-2">
              {programs.map((p) => (
                <div key={p.id} className="bg-[#0d0e0f] border border-white/[0.06] px-5 py-4 flex flex-wrap items-center gap-x-6 gap-y-2">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 flex-wrap">
                      <span className="text-white font-semibold text-sm">{p.name}</span>
                      <span className={`px-1.5 py-0.5 text-[10px] font-semibold tracking-wide ${statusCls(p.status)}`}>{p.status === "active" ? "Published" : p.status}</span>
                    </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <Link href={`/admin/programs/${p.id}`} className="text-[10px] tracking-[0.25em] uppercase font-semibold text-gray-500 border border-white/[0.08] px-3 py-1.5 hover:text-white hover:border-white/20 transition-colors">Edit</Link>
                    <button onClick={() => handleDelete(p.id, p.name)} className="text-[10px] tracking-[0.2em] uppercase font-semibold text-gray-700 border border-white/[0.05] px-3 py-1.5 hover:text-red-400 hover:border-red-500/30 transition-colors">Delete</button>
                  </div>
                </div>
              ))}
            </div>
