    ...
    <div>
      ...
      {showNewForm && (
        <div className="mb-8 bg-[#0d0e0f] border border-[#C9A24D]/30 p-6">
          <p className="text-white font-semibold text-sm mb-5 tracking-wide">New Workout Blueprint</p>
      ...
      <div className="flex items-center gap-3 mb-5">
        <span className="text-[10px] tracking-[0.4em] text-gray-600 uppercase font-semibold">Workout Templates</span>
        {!loading && (
      ...
      {!loading && templates.length > 0 && (
        <div className="space-y-2">
          {templates.map((t) => (
            <div
              key={t.id}
              className="bg-[#0d0e0f] border border-white/[0.06] px-5 py-4 flex flex-wrap items-center gap-x-6 gap-y-2 hover:border-white/10 transition-colors"
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 flex-wrap">
                  <span className="text-white font-semibold text-sm">{t.name}</span>
                  <span className={`px-1.5 py-0.5 text-[10px] font-semibold tracking-wide ${statusCls(t.status)}`}>
                    {t.status}
                  </span>
                </div>
                <div className="flex items-center gap-3 mt-1 flex-wrap">
                  ...
                  {t.primaryFocus && <span className="text-gray-600 text-[11px]">{t.primaryFocus}</span>}
                  {t.estimatedDurationMinutes && <span className="text-gray-600 text-[11px]">{t.estimatedDurationMinutes} min</span>}
                </div>
              </div>
